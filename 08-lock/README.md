...
```
<b> Как следует из лога третий сеанс 2060 был заблокирован первым сеансом 1689, который был заблокирован вторым сеансом 1794, который в свою очередь был заблокирован третим сеансом 2060.</b>
> Могут ли две транзакции, выполняющие единственную команду UPDATE одной и той же таблицы (без where), заблокировать друг друга?  

<b>Могут, если у транзакций будет разная последовательность обновления строк таблицы.</b>
...
(3 rows)
```
<b>Как видно, без установки хотя бы бессмысленного where i > 0 изменить план на index scan не удаётся.</b>
- Пример взаимоблокировки двух транзакций. Первый сеанс:
```sql
lock-session1=#begin;
BEGIN
lock-session1=*#select pg_backend_pid(), txid_current();
